...
# Load industry ticker list generated from comps script
f = open('data/ind-sample.json')
ind_sample = json.load(f)
...
for stock in ind_sample:
    ...
    valuation = np.npv(wacc, future_fcf) + tv * 1/(1 + wacc)**years
    print('Market cap valuation: ' + str(valuation) + '\n')
    print ('Actual market cap: ' + str(peer.fast_info['market_cap']) + '\n')
